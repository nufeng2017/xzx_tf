...
function editMedia(content) {
  ...
  if (content.type == 8) {
    ...
    if (obj.coverPic && obj.coverPic.trim() != '') {
      str += `<img class="imgtxt-img" src=${obj.coverPic} />`;
    }
    str += `<div class="imgtxt-title">${obj.houseTitle}</div>`;
    if (obj.price && obj.price.trim() != '') {
      str += `<div class="imgtxt-describe"><div class="imgtxt-buildarea">${obj.buildarea}</div><div class="imgtxt-price">${obj.price}</div></div>`;
    }
    str += `</a>`;
  } else {
