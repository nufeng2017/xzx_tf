...
import { generateFingerGuessImageFile, generateBigEmojiImageFile, generateRichTextNode, generateImageNode, calcTimeHeader, editMedia } from '../../utils/util.js'
...
let pageConfig = {
  ...
  onLoad: function (options) {
    console.log(options)
    if (!wx.getStorageSync('changeUid')){
      wx.redirectTo({
    ...
    console.log(detail)
    detail.houseType = options.houseType
    this.setData({
      chatTo,
      chatType,
      loginAccountLogo,
    ...
    let chatRecord = wx.getStorageSync('chat-record');
    if (chatRecord) {
      if (chatRecord[detail.data.houseId] != 'chatted'){
        this.sendHouse();
      }
    } else {
      this.sendHouse();
    ...
    app.globalData.nim.sendCustomMsg({
      ...
      content: JSON.stringify(content),
        ...
        if (msg.content){
          ...
          let record = wx.getStorageSync('chat-record');
          if (!record){
            wx.setStorageSync('chat-record', { [content.data.houseId]:'chatted'});
          } else {
            record[content.data.houseId] = 'chatted'
            wx.setStorageSync('chat-record', record);
          }
        }
        // 滚动到底部
        self.scrollToBottom()
  ...
  convertRawMessageListToRenderMessageArr(rawMsgList) {
    ...
    for (let time in rawMsgList) {
      ...
      if (rawMsg.type === 'custom' && JSON.parse(rawMsg['content'])['type'] === 1) {
        ...
        msgType = '贴图表情'
      } else if (rawMsg.type === 'custom' && JSON.parse(rawMsg['content'])['type'] === 8) {
        msgType = '楼盘'
      }else {
        msgType = rawMsg.type
      }
      ...
      switch (msgType) {
        ...
        case '自定义消息':
          ...
          break;
        case '楼盘':
          console.log(editMedia(rawMsg.content))
          specifiedObject = {
            nodes: editMedia(rawMsg.content)
          }
          break;
        case 'notification':
          specifiedObject = {
  ...
  checkHouse(e){//查看自定义图文消息，进入房源详情
    if (e.currentTarget.dataset.type === '楼盘'){
      let detail = this.data.detail;
      let url =''
      if (detail.houseType == 3){
        url = '/shangban/pages/detail/detail?detailtype=2&id=' + detail.data.houseId;
      }
      wx.navigateTo({
        url: url,
      })
