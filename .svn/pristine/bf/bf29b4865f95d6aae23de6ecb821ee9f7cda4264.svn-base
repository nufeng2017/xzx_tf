...
let sex = [];//性别
let fitment_name = [];
let min_periods = ['0个月'];//起租期
let free_periods = ['0个月'];//免租期
for (let i = 1; i <= 99; i++) {
  min_periods.push(i+'个月');
  if (i <= 36){
    free_periods.push(i + '个月');
  }
  floor.push(`${i}层`);
...
Page({
  ...
  onLoad(options){
    ...
    defaultPublishObj = JSON.parse(JSON.stringify(this.data.publishObj));
    defaultPublishObj.house_type = this.resetObj(wx.getStorageSync('initConfig').house_type_map)[options.houseType];
    this.setData({
      ['publishObj.uid']: app.getUtil.cacheGet("changeUid"),
      topArr: topArr,
  ...
  topContent(houseType){//判断此房源类型有哪些选项
    switch (houseType){
      ...
      case '写字楼':
        return [topArr[5], topArr[1],topArr[2]];
      case '商铺':
        return [topArr[4], topArr[3], topArr[2]];
      case '厂房仓库':
        return [topArr[1],topArr[2]];
      case '别墅':
        return [topArr[0], topArr[1], topArr[2]];
    }
  },
  ...
  setPublishObj(data, options){//提交和返回数据不一样，需要重新赋值
   console.log(data)
    ...
    o.min_periods = data.min_periods;//起租期
    o.free_periods = data.free_periods;//免租期
    o.transferfee = data.transferfee;//转让费
    o.rent_items = data.rent_items;//租金包含
    o.checkin_time = data.checkin_time;//入住时间
    o.look_house_time = parseFloat(data.look_house_time);//看房时间
    o.house_special = data.house_special ? data.house_special.split(',') : [];//房屋两点
    o.rental_condition = data.rental_condition?data.rental_condition.split(','):[];//出租要求
  ...
  setPickerViewData(o){//装填滚动选择窗口的数据
    ...
    switch (o.itemType){
      ...
      case '可分割':
        return { listNum: [this.data.isFaceStreet], value: value };
      case '起租期':
        return { listNum: [this.data.min_periods], value: value };
      case '免租期':
        return { listNum: [this.data.free_periods], value: value };
      case '类别':
        return { listNum: [Object.values(this.data.iniconfig.office_type_map)], value:value };
    }
  },
  floorType(type){
  ...
  checkValueOk(){//再次检查数值是否正确
    ...
    var reg = /\.$/;
    publishObj.build_area = typeof publishObj.build_area ==='string'?publishObj.build_area.replace(reg,''):'';
    publishObj.price = typeof publishObj.price === 'string' ?publishObj.price.replace(reg, ''):'';
    publishObj.fee = typeof publishObj.fee === 'string' ?publishObj.fee.replace(reg, ''):'';
    this.setData({
  ...
  isLegitimateValue(value){//检查是否是合法数值
    ...
    if (this.data.itemType === '面积' || this.data.itemType === '月租金' || this.data.itemType === '物业费'){
      return typeof value ==='string'?value.replace(reg, ''):'';
    } else {
      return typeof value === 'string' ? value.replace(reg1, '') : '';
    }
  },
  ...
  bindChange(e){//滚动选项事件
    if (e.type === 'change'){
      ...
      for (let i = 0 ; i < val.length ; i ++){
        ...
        if (this.data.itemType === '商铺性质' || this.data.itemType === '性质' || this.data.itemType === '商铺类型' || this.data.itemType === '类别') {
          o[p[i]] = val[i]+1;
          break;
        }
  ...
  checkData(){//检查数据是否填写完毕
    ...
    var arr = [
      ...
      { value: 'fee', str: '物业费' },
      { value: 'min_periods', str: '起租期' },
      { value: 'free_periods', str: '免租期' },
      { value: 'office_type', str: '类别' }
    ];
    if (this.data.publishObj.house_type == 1 || this.data.publishObj.house_type == 2){//住房
      var mustItem = [0, 1, 2, 3, 4,5, 7, 8];
      if (obj.rent_type ==2){
        var mustItem = [0, 1, 2, 3, 4, 5, 6, 7,8];
      }
    }
    if (this.data.publishObj.house_type == 4) {//商铺
      var mustItem = [0,1, 4, 5, 7, 8, 9, 10];
    }
    if (this.data.publishObj.house_type == 3) {//写字楼
      var mustItem = [0,1,3, 4, 5, 7, 8, 10,11,12,13];
    }
    if (this.data.publishObj.house_type == 5) {//厂房仓库
      var mustItem = [0,1,3, 4, 5, 7];
    }
    if (this.data.publishObj.house_type == 6) {//车位
      var mustItem = [0,1, 5, 7];
    }
    for (var i =0 ;i < mustItem.length;i++){
      if (obj[arr[mustItem[i]].value] === '' || obj[arr[mustItem[i]].value].length==0){
  ...
  reset(){//重置数据
    console.log(defaultPublishObj)
    this.setData({
      publishObj: JSON.parse(JSON.stringify(defaultPublishObj))
    });
    this.saveData();
  },
  isShowNewModel(e){
    let rent_items = this.data.publishObj.rent_items?this.data.publishObj.rent_items.split(','):[];
    let o = {};
    rent_items.map((item)=>{
      o[item] = 1;
    });
    this.setData({
      isShowNewModel: e.currentTarget.dataset.modelstatus,
      dataTemporary: rent_items,
      dataTemporaryObj:o
    })
  },
  checkboxChange(e){
    ...
    this.setData({
      dataTemporary: e.detail.value
    });
  },
  finishSel(){
    this.setData({
      ['publishObj.rent_items']: this.data.dataTemporary.join(','),
